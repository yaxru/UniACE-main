...
export default function MidExamDatesPage() {
  ...
  async function fetchEntries() {
    setLoading(true);
    try {
      const { data } = await api.get('/exam-date-entries', { params: { examType: 'mid' } });
      setEntries(data);
    } catch {
      setError('Failed to load mid exam dates.');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleSubmit(e) {
    ...
    try {
      const payload = { examType: 'mid', ...form };
      ...
      setSuccess(editingEntryId ? 'Mid exam date updated successfully.' : 'Mid exam date added successfully.');
      setForm({ ...emptyForm });
      setEditingEntryId(null);
    } catch (err) {
      setError(err.response?.data?.message || `Failed to ${editingEntryId ? 'update' : 'add'} mid exam date.`);
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleDelete(entryId) {
    if (!window.confirm('Delete this mid exam date?')) return;
    ...
    try {
      ...
      setSuccess('Mid exam date deleted successfully.');
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to delete mid exam date.');
    }
  }
  ...
  return (
    <div className="modules-page-card">
      <div className="modules-page-header">
        <h2>Mid Exam Dates</h2>
        <p>{editingEntryId ? 'Edit mid exam date for a module.' : 'Add mid exam dates for modules.'}</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="exam-form-box">
        ...
        <div className="form-group">
          <label htmlFor="examDate">Mid Exam Date</label>
          <input
            id="examDate"
            name="examDate"
            type="date"
        ...
        <div className="form-group">
          <label htmlFor="examStartTime">Mid Exam Start Time</label>
          <input
            id="examStartTime"
            name="examStartTime"
            type="time"
        ...
        <div className="form-group">
          <label htmlFor="examEndTime">Mid Exam End Time</label>
          <input
            id="examEndTime"
            name="examEndTime"
            type="time"
        ...
        <div className="modules-actions">
          ...
          <button type="submit" className="btn btn-primary" disabled={saving}>
            {saving ? 'Saving…' : editingEntryId ? 'Update Mid Exam Date' : 'Add Mid Exam Date'}
          </button>
        </div>
      </form>

      <div className="module-latest-box">
        <h3>Saved Mid Exam Dates</h3>
        {isLicUser && <p className="module-upload-subtitle">You can view mid exam dates added by all LIC users and students.</p>}

        {loading && <p className="state-msg" style={{ textAlign: 'left', padding: 0 }}>Loading schedules…</p>}
        {!loading && entries.length === 0 && <p className="home-modules-empty">No mid exam dates added yet.</p>}
